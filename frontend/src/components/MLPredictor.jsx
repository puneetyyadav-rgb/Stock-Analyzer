...
export default function MLPredictor({ symbol }) {
  ...
  if (loading) {
    return (
      <Panel className="flex items-center justify-center py-12 text-zinc-500">
  ...
  if (error) {
    return (
      <Panel className="py-6 border-red-900/40 bg-red-950/10">
    ...
    <div className="border border-purple-900/50 bg-[#0c0c0e] rounded shadow-sm overflow-hidden mb-3">
      ...
      <div className="bg-purple-900/20 px-4 py-2 border-b border-purple-900/50 flex items-center gap-2">
        ...
        <h3 className="text-xs font-semibold tracking-wider uppercase text-purple-300">
          Experimental Mathematical Forecasting
        </h3>
        <span className="ml-auto text-[9px] text-purple-400/70 tracking-widest uppercase">
      ...
      <div className="p-4 grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="lg:col-span-1 flex flex-col justify-center space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-[#121215] p-2 rounded border border-zinc-800/60">
              <p className="text-[9px] tracking-widest uppercase text-zinc-500 mb-0.5">7-Day Target</p>
              <p className="text-sm font-mono text-zinc-200">₹{data.projected7D}</p>
            </div>
            <div className="bg-[#121215] p-2 rounded border border-zinc-800/60">
              <p className="text-[9px] tracking-widest uppercase text-zinc-500 mb-0.5">30-Day Target</p>
              <p className="text-sm font-mono text-zinc-200">₹{data.projected30D}</p>
            </div>
          </div>

          <div className="bg-[#121215] p-3 rounded border border-zinc-800/60">
            <div className="flex items-center gap-2 mb-1">
              <p className="text-[10px] tracking-widest uppercase text-zinc-400">Backtest Accuracy</p>
              {data.backtestAccuracy < 80 && (
                <AlertTriangle size={12} className="text-amber-400" title="High historical error" />
              )}
            </div>
            <div className="flex items-end gap-2">
              <span className={`text-xl font-mono ${data.backtestAccuracy >= 90 ? "text-emerald-400" : data.backtestAccuracy >= 80 ? "text-amber-400" : "text-red-400"}`}>
                {data.backtestAccuracy}%
              </span>
              <span className="text-[10px] text-zinc-600 mb-1">MAPE: {data.mape}%</span>
            </div>
            <p className="text-[9px] text-zinc-500 mt-1 leading-tight">
              Based on last 30 days out-of-sample validation.
            </p>
          </div>
        </div>

        {/* Right Side: Chart */}
        <div className="lg:col-span-3 min-h-[240px] relative">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
              ...
              <Tooltip
                ...
                formatter={(val, name) => {
                  if (name === "range") return [`₹${val[0]} - ₹${val[1]}`, "90% Confidence"];
                  if (name === "close") return [`₹${val}`, "Actual Price"];
                  if (name === "forecast") return [`₹${val}`, "Predicted Price"];
                  return [val, name];
                }}
              />
